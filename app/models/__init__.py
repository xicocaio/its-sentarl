...
def load_model(total_timesteps, config: Config, window: str, window_roll: int):
    ...
    abs_filename = os.path.join(save_path, save_name + '_{}_steps.zip'.format(eval_freq))
...
def _prepare_save_callback(eval_freq, save_model, config: Config, window: str, window_roll: int, seed: int,
    ...
    if save_model:
        ...
        if overwrite_file or not os.path.isfile(os.path.join(save_path, save_name + '_{}_steps.zip'.format(eval_freq))):
            return CheckpointCallback(save_freq=eval_freq, save_path=save_path, name_prefix=save_name)
